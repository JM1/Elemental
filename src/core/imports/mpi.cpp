...
#define MPI_PROTO(T) \
  ...
  EL_NO_RELEASE_EXCEPT; \
  template void Broadcast<T>( T* buf, int count, int root, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  ...
  EL_NO_RELEASE_EXCEPT; \
  template void AllGather<T>( const T* sbuf, int sc, T* rbuf, int rc, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  ...
  ( const T* sbuf, int sc, \
          T* rbuf, int rc, int root, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void Scatter<T>( T* buf, int sc, int rc, int root, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  ...
  EL_NO_RELEASE_EXCEPT; \
  template void AllReduce<T>( T* buf, int count, Op op, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
...
#define MPI_PROTO_COMPLEX(T) \
  template bool Test( Request<Complex<T>>& request ) EL_NO_RELEASE_EXCEPT; \
  template void Wait( Request<Complex<T>>& request ) EL_NO_RELEASE_EXCEPT; \
  template void Wait( Request<Complex<T>>& request, Status& status ) \
  EL_NO_RELEASE_EXCEPT; \
  template void WaitAll( int numRequests, Request<Complex<T>>* requests ) \
  EL_NO_RELEASE_EXCEPT; \
  template void WaitAll \
  ( int numRequests, Request<Complex<T>>* requests, Status* statuses ) \
  EL_NO_RELEASE_EXCEPT; \
  template int GetCount<Complex<T>>( Status& status ) EL_NO_RELEASE_EXCEPT; \
  template void TaggedSend<Complex<T>> \
  ( const Complex<T>* buf, int count, int to, int tag, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void Send<Complex<T>>( const Complex<T>* buf, int count, int to, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void TaggedSend( Complex<T> b, int to, int tag, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void Send<Complex<T>>( Complex<T> b, int to, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void TaggedISend<T> \
  ( const Complex<T>* buf, int count, int to, int tag, Comm comm, Request<Complex<T>>& request ) \
  EL_NO_RELEASE_EXCEPT; \
  template void ISend<Complex<T>> \
  ( const Complex<T>* buf, int count, int to, Comm comm, Request<Complex<T>>& request ) \
  EL_NO_RELEASE_EXCEPT; \
  template void TaggedISend<Complex<T>> \
  ( Complex<T> buf, int to, int tag, Comm comm, Request<Complex<T>>& request ) \
  EL_NO_RELEASE_EXCEPT; \
  template void ISend( Complex<T> buf, int to, Comm comm, Request<Complex<T>>& request ) \
  EL_NO_RELEASE_EXCEPT; \
  template void TaggedISSend<Complex<T>> \
  ( const Complex<T>* buf, int count, int to, int tag, Comm comm, Request<Complex<T>>& request ) \
  EL_NO_RELEASE_EXCEPT; \
  ...
  EL_NO_RELEASE_EXCEPT; \
  template void TaggedISSend<Complex<T>> \
  ( Complex<T> b, int to, int tag, Comm comm, Request<Complex<T>>& request ) \
  EL_NO_RELEASE_EXCEPT; \
  template void TaggedRecv<Complex<T>> \
  ( Complex<T>* buf, int count, int from, int tag, Comm comm ) EL_NO_RELEASE_EXCEPT; \
  template void Recv<Complex<T>>( Complex<T>* buf, int count, int from, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template Complex<T> TaggedRecv<Complex<T>>( int from, int tag, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template Complex<T> Recv<Complex<T>>( int from, Comm comm ) EL_NO_RELEASE_EXCEPT; \
  template void TaggedIRecv<Complex<T>> \
  ( Complex<T>* buf, int count, int from, int tag, Comm comm, Request<Complex<T>>& request ) \
  EL_NO_RELEASE_EXCEPT; \
  template void IRecv<Complex<T>> \
  ( Complex<T>* buf, int count, int from, Comm comm, Request<Complex<T>>& request ) \
  EL_NO_RELEASE_EXCEPT; \
  template Complex<T> TaggedIRecv<Complex<T>> \
  ( int from, int tag, Comm comm, Request<Complex<T>>& request ) EL_NO_RELEASE_EXCEPT; \
  template Complex<T> IRecv<Complex<T>>( int from, Comm comm, Request<Complex<T>>& request ) \
  EL_NO_RELEASE_EXCEPT; \
  template void TaggedSendRecv<Complex<T>> \
  ( const Complex<T>* sbuf, int sc, int to,   int stag, \
          Complex<T>* rbuf, int rc, int from, int rtag, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void SendRecv<Complex<T>> \
  ( const Complex<T>* sbuf, int sc, int to, \
          Complex<T>* rbuf, int rc, int from, Comm comm ) EL_NO_RELEASE_EXCEPT; \
  template Complex<T> TaggedSendRecv<Complex<T>> \
  ( Complex<T> sb, int to, int stag, int from, int rtag, Comm comm ); \
  template Complex<T> SendRecv( Complex<T> sb, int to, int from, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void TaggedSendRecv<Complex<T>> \
  ( Complex<T>* buf, int count, int to, int stag, int from, int rtag, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void SendRecv<Complex<T>> \
  ( Complex<T>* buf, int count, int to, int from, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void Broadcast<T>( Complex<T>* buf, int count, int root, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void Broadcast<Complex<T>>( Complex<T>& b, int root, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void IBroadcast<Complex<T>> \
  ( Complex<T>* buf, int count, int root, Comm comm, Request<Complex<T>>& request ); \
  template void IBroadcast<Complex<T>> \
  ( Complex<T>& b, int root, Comm comm, Request<Complex<T>>& request ); \
  template void Gather<T> \
  ( const Complex<T>* sbuf, int sc, Complex<T>* rbuf, int rc, int root, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void IGather<Complex<T>> \
  ( const Complex<T>* sbuf, int sc, \
          Complex<T>* rbuf, int rc, \
    int root, Comm comm, Request<Complex<T>>& request ); \
  template void Gather<T> \
  ( const Complex<T>* sbuf, int sc, \
          Complex<T>* rbuf, const int* rcs, const int* rds, int root, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void AllGather<T>( const Complex<T>* sbuf, int sc, Complex<T>* rbuf, int rc, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void AllGather<T> \
  ( const Complex<T>* sbuf, int sc, \
  ...
  EL_NO_RELEASE_EXCEPT; \
  template void Scatter<T>( Complex<T>* buf, int sc, int rc, int root, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void AllToAll<T> \
  ( const Complex<T>* sbuf, int sc, \
  ...
  ( const Complex<T>* sbuf, const int* scs, const int* sds, \
          Complex<T>* rbuf, const int* rcs, const int* rds, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template vector<Complex<T>> AllToAll<Complex<T>> \
  ( const vector<Complex<T>>& sendBuf, \
    const vector<int>& sendCounts, \
    const vector<int>& sendOffs, \
    Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  ...
  EL_NO_RELEASE_EXCEPT; \
  template Complex<T> Reduce<Complex<T>>( Complex<T> sb, Op op, int root, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template Complex<T> Reduce<Complex<T>>( Complex<T> sb, int root, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void Reduce<Complex<T>>( Complex<T>* buf, int count, Op op, int root, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void Reduce<Complex<T>>( Complex<T>* buf, int count, int root, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void AllReduce<T> \
  ( const Complex<T>* sbuf, Complex<T>* rbuf, int count, Op op, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void AllReduce( const Complex<T>* sbuf, Complex<T>* rbuf, int count, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template Complex<T> AllReduce( Complex<T> sb, Op op, Comm comm ) \
  EL_NO_RELEASE_EXCEPT;\
  template Complex<T> AllReduce( Complex<T> sb, Comm comm ) \
  EL_NO_RELEASE_EXCEPT;\
  template void AllReduce<T>( Complex<T>* buf, int count, Op op, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void AllReduce<Complex<T>>( Complex<T>* buf, int count, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void ReduceScatter<Complex<T>>( Complex<T>* sbuf, Complex<T>* rbuf, int rc, Op op, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void ReduceScatter( Complex<T>* sbuf, Complex<T>* rbuf, int rc, Comm comm ) \
  EL_NO_RELEASE_EXCEPT;\
  template Complex<T> ReduceScatter<Complex<T>>( Complex<T> sb, Op op, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template Complex<T> ReduceScatter<Complex<T>>( Complex<T> sb, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void ReduceScatter<Complex<T>>( Complex<T>* buf, int rc, Op op, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void ReduceScatter<Complex<T>>( Complex<T>* buf, int rc, Comm comm ) \
  EL_NO_RELEASE_EXCEPT;\
  template void ReduceScatter<Complex<T>> \
  ( const Complex<T>* sbuf, Complex<T>* rbuf, const int* rcs, Op op, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void ReduceScatter<Complex<T>> \
  ( const Complex<T>* sbuf, Complex<T>* rbuf, const int* rcs, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void Scan<Complex<T>>( const Complex<T>* sbuf, Complex<T>* rbuf, int count, Op op, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void Scan<Complex<T>>( const Complex<T>* sbuf, Complex<T>* rbuf, int count, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template Complex<T> Scan<Complex<T>>( Complex<T> sb, Op op, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template Complex<T> Scan<Complex<T>>( Complex<T> sb, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void Scan<Complex<T>>( Complex<T>* buf, int count, Op op, Comm comm ) \
  EL_NO_RELEASE_EXCEPT; \
  template void Scan<Complex<T>>( Complex<T>* buf, int count, Comm comm ) \
  EL_NO_RELEASE_EXCEPT;
